...
local tasktext = {}
tasktext[1] = "TASK 1: Browse all highlighted nodes"
tasktext[2] = "TASK 2: Count all highlighted nodes"
tasktext[3] = "TASK 3: Verify that the node that belongs to both groups"
tasktext[4] = "TASK 4: Verify that the node that belongs to only one group"
tasktext[5] = "TASK 5: Find a node that is connected to both groups"
...
local conditions = {"3D", "2D", "2.5DH", "2.5D"}
local questions = {}
local answers = {}
...
function win:draw()
    ...
    if(question < MAXQ+1 ) then
		if(boolbegin) then 
			...
			local tsktext = tasktext[crr_task]
			questionlabels.color = {0.8, 0.2, 0.2}
...
answers[17] = {"TT", "KD", "None"}
answers[18] = {"TT", "KD", "None"}
answers[19] = {"TT", "KD", "None"}
answers[20] = {"TT", "KD", "None"}
