...
local file = "/data/coauthor_large.xml"
...
local effectivedim = win.dim
effectivedim[1] = win.dim[1]*0.5
	...
	for l=1, tpd:graphsize() do
		...
		if(intersects) then
			if( ind ~= selnodes[1] and ind ~=selnodes[2]) then 
				...
				local p = tpd:graphnodepos(ind)
				graphlabels:draw_3d(effectivedim, {p[1], p[2], p[3]}, labelstr)
			end
			break
		end
	    end
...
function win:draw(eye)
    ...
    guilabels:draw_ortho(effectivedim, {35, 30, 0}, "Move Node")
	guilabels:draw_ortho(effectivedim, {35, 50, 0}, "Drag Node")
	guilabels:draw_ortho(effectivedim, {35, 70, 0}, "Drag Plane")
	
	guilabels:draw_ortho(effectivedim, {10, 110, 0}, "Edge Color Green Channel")
	guilabels:draw_ortho(effectivedim, {10, 160, 0}, "Line Thickness")
	guilabels:draw_ortho(effectivedim, {10, 210, 0}, "Point Size")
...
function win:resize()
    effectivedim = self.dim
    effectivedim[1]= self.dim[1]*0.5
    cam:resize(effectivedim)
	gui:resize(effectivedim)
end
