...
local data = "/data/coauthor_train.xml"
local sourcepath = script.path .. data
...
function saveGraph()
    ...
    local fileN = "graphtrain_pos.txt"
    if(layout3d) then fileN = "graphtrain_3Dpos.txt" end
...
local 
function question2(node1, node2, case)
...
function getOSC() 
	for msg in oscin:recv() do	
		...
		elseif(msg.addr == "/setstereo") then 
			local boolst =  unpack (msg)
			if(win ~= nil and cam ~= nil) then 
				win.stereo = boolst
...
function testpos()
	...
	for l=1, tpd:graphsize() do
		local ind = l-1
		local p = tpd:graphnodepos(ind)	
		local wc = glu.Project(p)
		print(l, wc[1], wc[2])
		windowcoords[l] = {wc[1], ( win.dim[2] - wc[2] )}
	end
end


local 
function calcRepulse()
	for l=1, 32 do
		for n=1, 32 do
			if(n ~= l) then 
				local p1 = { windowcoords[l][1], windowcoords[l][2], 0.0 }
				local p2 = { windowcoords[n][1], windowcoords[n][2], 0.0 }

				local dist = vec3.sub(p1, p2)
				local dismag = vec3.mag(dist) 
				dist = vec3.normalize(dist)
				local repforce = vec3.scale (dist , ( 0.025 / dismag ) )
				
				--accumulate disp
				disp[l] = vec3.add (disp[l], repforce)					
			end
		end
	end
end
...
function step(temp)
	...
	for l=1, 32 do
		local dis = disp[l]
		local dismag = vec3.mag(dis)
		dis = vec3.normalize(dis)
		local scaled = vec3.scale ( dis , math.min ( dismag, temp ) )
		local pos = { windowcoords[l][1], windowcoords[l][1], 0.0 }
		local newpos = vec3.add ( pos , scaled )
		--windowcoords[l]  = newpos
	end
end


local boolwcoords = false
local boolstep = false
local wtemp = 0.5
local wst = 1
...
function win:draw(eye)
    ...
    if(boolstep) then
    	if(wst < MAXSTEP and wtemp > 0.0001) then 
			step(wtemp)
	 		wtemp = wtemp * 0.98
	 		wst = wst+1 	
		end
    ...
    gl.PointSize(5.0)
    gl.Begin(GL.POINTS)
    for k, v in pairs(windowcoords) do
		gl.Vertex(v[1], v[2], 0.0)
	end
	gl.End()
	sketch.leave_ortho()
	...
	guilabels:draw({10, 100, 0}, "Eye Seperation")
...
function win:key(event, key)
	 ...
	 if(event == "down") then 
		...
		elseif(key == 98 or key == 66) then --B
			boolstep = not boolstep
