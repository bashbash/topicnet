...
local filepath = script.path .. file
...
--tpd:loadData(sourcepath3, "author")
tpd:loadData(filepath, "face")



-------------------------------------------------
local context = "for pics"
win = Window{
	title = context, 
	origin = {0, 0}, 
	dim = {500, 500},
...
local blue2 = {98/255, 152/255, 205/255}
local highcol = {}
...
function win:draw(eye)
	...
	if(not blockview) then
		...
		if(draw3d) then 
			...
			shader:bind()
				tpd:drawGraphNodes(true, 0.001*pointscale)
			shader:unbind()
		...
		for i=1,2 do 
			...
			if(selectednode~=nil and selectednode > -1) then
				...
				if(draw3d) then 
					...
					gl.PushMatrix()
						gl.Translate(p)  
						gl.Scale(0.0015*pointscale, 0.0015*pointscale, 0.0015*pointscale)
